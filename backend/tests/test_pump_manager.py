...
def test_pump_manager(arduino_fxt: ae.EmbeddedArduino):
    time = []
    ...
    if DEBUG:
        ...
        axs1 = list()
        for channel, ax0 in enumerate(axs0):
            ax1 = ax0.twinx()
    ...
    pump_manager = pm.PumpManager(
        arduino_fxt,
        sp.SmartPumpSettings(
    ...
    while perf_counter() - T0 < 30:
        statuses = pump_manager.statuses

        time.append(perf_counter() - T0)

        for channel in statuses:
            rel_humidity_pcnts[channel].append(statuses[channel].rel_humidity_pcnt)
            pump_states[channel].append(statuses[channel].pump_running)

            if DEBUG:
                axs0[channel].plot(time, rel_humidity_pcnts[channel], ".-b")
                axs1[channel].plot(time, pump_states[channel], ".-r")
                plt.pause(0.001)

    pump_manager.interrupt()
